...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomTelHdle;                    /**< Telemetry handle */
  uint16_t  CustomTel_EnHdle;                  /**< TEL_EN handle */
...
#define COPY_TELEMETRY_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_TEL_EN_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomTel_EnHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  COPY_TELEMETRY_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomTelHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: TEL, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: TEL , handle = 0x%04x \n\r", CustomContext.CustomTelHdle);
  }

  /**
   *  TEL_EN
   */
  COPY_TEL_EN_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomTelHdle,
                          UUID_TYPE_128, &uuid,
                          SizeTel_En,
                          CHAR_PROP_WRITE,
                          ...
                          &(CustomContext.CustomTel_EnHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : TEL_EN, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : TEL_EN , handle = 0x%04x \n\r", CustomContext.CustomTel_EnHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TEL_EN:
      ret = aci_gatt_update_char_value(CustomContext.CustomTelHdle,
                                       CustomContext.CustomTel_EnHdle,
                                       0, /* charValOffset */
                                       SizeTel_En, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value TEL_EN command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value TEL_EN command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TEL_EN:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomTelHdle, CustomContext.CustomTel_EnHdle, SizeTel_En, pPayload);
